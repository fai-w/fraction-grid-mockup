...
let learnQueue = [];   // holds shuffled items
let learnIndex = 0;    // current item
const MAX_LEARN_COUNT = 5;
...
function initLearnSequence() {
    learnQueue = getNewUnseenProblems(itemBank, MAX_LEARN_COUNT);

    if (learnQueue.length < MAX_LEARN_COUNT) {
        alert("คุณได้ฝึกครบทุกโจทย์แล้ว! จะเริ่มใหม่หรือไม่?");
        seenFractions.clear();
        learnQueue = getNewUnseenProblems(itemBank, MAX_LEARN_COUNT);
    }

    function getNewUnseenProblems(itemBank, limit) {
        const freshItems = [];
        for (const item of shuffle(itemBank)) {
            const key = `${item.num}/${item.den}`;
            if (!seenFractions.has(key)) {
                seenFractions.add(key);
                freshItems.push(item);
                if (freshItems.length === limit) break;
            }
        }
        return freshItems;
    }
...
document.getElementById('stay-learn').onclick = () => {
...
function askForNumerator() {
    ...
    const decimalRow = document.getElementById('decimal-row');
    ...
    if (isCorrect) {
        ...
        document.getElementById('decimal-row').innerHTML = `
        <div style="text-align:center; margin-top: 12px;">
            และ \\(\\frac{${START_NUM}}{${START_DEN}}\\) เขียนในรูปทศนิยมได้ว่า <strong>${decimalExpected}</strong>
        </div>
    `;
        MathJax.typesetPromise([document.getElementById('decimal-row')]);
        ...
        submitBtn.onclick = () => {
            ...
            const isValidFrac = /^[0-9]+$/.test(input.value.trim());

            const decimalVal = document.getElementById('decimal-input').value.trim();
            const isValidDec = /^[0-9]*\.?[0-9]+$/.test(decimalVal);

            const decimalCorrect = parseFloat(decimalVal) === parseFloat(decimalExpected);

            if (isValidFrac && val === expected && isValidDec && decimalCorrect) {
                isCorrect = true;
                disableGridControls();
                ...
                askForNumerator();  // will go into correct-state rendering
            } else {
...
function showLearnCompleteModal() {
    const modal = document.getElementById('review-complete-modal');
    const text = modal.querySelector('p');
    text.textContent = 'เก่งมาก! พร้อมจะฝึกฝนแล้วโดยไม่ใช้รูปหรือยัง?';

    const goBtn = document.getElementById('go-practice');
    const stayBtn = document.getElementById('stay-learn');

    goBtn.textContent = 'ไปเลย';
    stayBtn.textContent = 'ทำซ้ำอีกหน่อย';
...
function nextProblem() {
    if (currentProblemIndex >= learnQueue.length) {
        showLearnCompleteModal();  // like review modal
        return;
    }

    const item = learnQueue[currentIndex++];
    console.log('Loaded item:', item);  // ← THIS LINE
    ...
    document.getElementById('problem-intro').innerHTML =
        `เขียน \\(\\frac{${START_NUM}}{${START_DEN}}\\) ในรูปทศนิยม`;
    fracDisp.innerHTML =
        `\\(\\frac{${START_NUM}}{${START_DEN}} = \\frac{\\Box}{${TARGET_DEN}}\\)`;

    safeTypeset([document.getElementById('problem-intro'), fracDisp]).then(() => {
        pastStates.push({
            rows: cfg.rows,
            cols: cfg.cols,
            shadeBy: cfg.shadeBy,
